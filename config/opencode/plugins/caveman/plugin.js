...
export const CavemanPlugin = async (_ctx) => {
  ...
  return {
  // Dynamically assign small_model to subagents that don't need the full
  // default model. Keeps agent files DRY — change small_model once, all follow.
  config: (cfg) => {
    if (!cfg.small_model) return;
    const subagents = ['cavecrew-investigator', 'cavecrew-builder', 'cavecrew-reviewer', 'explore'];
    for (const name of subagents) {
      if (!cfg.agent) cfg.agent = {};
      if (!cfg.agent[name]) cfg.agent[name] = {};
      cfg.agent[name].model = cfg.small_model;
    }
  },
